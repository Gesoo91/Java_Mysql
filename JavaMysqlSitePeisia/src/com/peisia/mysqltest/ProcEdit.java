...
//		이를 방지하기 위해 Prepared Statement를 사용하여 미리 컴파일하고 실행시에는 파라미터를 전달하는 방식으로 하는게 좋다.
	    ...
	    while(true) {
		    ...
		    if(editNoInt <1 || editNoInt > postCountNo) { //유효성검사 지워야할수도잇음. 아니면 b_no의 마지막 숫자를 나오게하고 b_no 없는부분은 다시입력하게하던가.
		    	Cw.wn("유효하지 않은 글번호입니다. 1번부터"+postCountNo+"의 숫자를 입력해주세요.");
		    	continue;
		    }
		    String edTitle = Ci.rl("제목을 입력해주세요:");
		    String edId = Ci.rl("작성자id를 입력해주세요:");
		    ...
		    try {
		        ...
		        PreparedStatement pstmt = Db.con.prepareStatement(query); //객체생성, Db.con은 데이터 베이스 연결을 나타내는 Connection 객체.
